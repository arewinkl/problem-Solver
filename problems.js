...
//? This turnery simplifies the solution to this problem to a single line of code.
function even_or_odd(number) {
...
//TODO this function uses closures which privatizes what the functions can access, but since var is not mutable you have to use let which is mutable, but with the closure still has access to that variable. Look over closures more to get a better grasp on what is going on.
function buildFun(n) {
  var res = [];
...
//? a closure works much like encapsulation, in that it keeps whats in the inner function private from the main function counter().
function counter() {
  let count = 1;
...
// n the planet named Hoiyama, scientists are trying to find the weights of the mountains.
...
function findDifference(a, b) {
  const first = a[0] * a[1] * a[2];
  const second = b[0] * b[1] * b[2];
